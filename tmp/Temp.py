...
def deal_data_for_Deng_and_Ryu():
    drug2smile = dict()
    df_d2s = pd.read_csv("src/data/Deng's dataset/drug_smiles.csv")
    for i in range(len(df_d2s)):
        drug, smile = df_d2s.iloc[i]
        drug2smile[drug] = smile

    def deal(k, kind):
        df = pd.read_csv(f"src/data/Ryu's dataset/{k}/ddi_{kind}1.csv")
...
# def do_draw_confusion_matrix_heatmap():
#     model = MyModel()
...
#     model.to(args.device)
#
#     _, _, testDateLoader = init_data(4)
#
#     y_true, y_pred = test(model, testDateLoader)
#
...
#     ax.xaxis.tick_top()
#
#     plt.savefig("Out/confusion.png")
#     print("confusionMatrix.png has saved")


# do_draw_confusion_matrix_heatmap()


draw_mol("ClCCNP1(=O)OCCCN1CCCl")
