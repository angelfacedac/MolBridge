...
def init_model_and_tools():
    ...
    log_root_path = os.path.join("Out", f"{logdir_name}", f"{args.batch}_{str(args.lr).replace('.', 'd', 1)}")
...
def valida(model, dataloader, loss_fn, epoch, writer, ACC_list, F1_list, two_class=False):
    ...
    with torch.no_grad():
        ...
        for data in dataloader:
            ...
            sum_loss += loss.item()
            y_true.append(targets.int())
            if two_class:
                y_pred.append((nn.functional.sigmoid(P) > 0.5).int())
            else:
                y_pred.append(torch.argmax(P, dim=1).int())
            y_prob.append(P)
