...
def collate_fn(batch):
    ...
    embeds = torch.zeros(len(batch), 2 * args.clip_n_atom, args.atom_dim)
    adjs = torch.zeros(len(batch), 2 * args.clip_n_atom, 2 * args.clip_n_atom)
    masks = torch.zeros(len(batch), 2 * args.clip_n_atom)
    cnn_masks = torch.zeros(len(batch), 2 * args.clip_n_atom, 2 * args.clip_n_atom)
...
def draw_confusion_matrix_heatmap(y_true_file_path, y_pred_file_path, save_path):
    y_true = np.load(y_true_file_path)[-1]
    y_pred = np.load(y_pred_file_path)[-1]

    cm = confusion_matrix(y_true, y_pred)

    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues')

    plt.suptitle('Confusion Matrix Heatmap', y=0.05)
    ...
    print("confusionMatrix.png has saved")
